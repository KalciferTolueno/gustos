...
export default async function AdminPage() {
  ...
  const db = getDb();
  const [pending, [usage], coverage, recentRuns, recentSearches] = await Promise.all([
  ...
  return <main className="min-h-screen bg-[#0b0c0e] px-4 py-8 text-zinc-100 sm:px-8">
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <div className="overflow-hidden rounded-2xl border border-white/10"><h2 className="border-b border-white/10 p-4 font-medium">Ejecuciones recientes</h2><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="text-zinc-500"><tr><th className="p-3">Fecha</th><th>Estado</th><th>Tipo</th><th>Búsquedas</th><th>Resultados</th></tr></thead><tbody>{recentRuns.map((run) => <tr key={run.id} className="border-t border-white/5"><td className="p-3 text-zinc-400">{formatDate(run.startedAt)}</td><td>{run.status}</td><td>{run.kind}</td><td>{run.searches}</td><td>{run.published}</td></tr>)}</tbody></table></div></div>
        <div className="overflow-hidden rounded-2xl border border-white/10"><h2 className="border-b border-white/10 p-4 font-medium">Búsquedas de usuarios</h2><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="text-zinc-500"><tr><th className="p-3">Fecha</th><th>Consulta</th><th>Estado</th><th>Resultados</th></tr></thead><tbody>{recentSearches.map((search, index) => <tr key={`${search.createdAt.toISOString()}-${index}`} className="border-t border-white/5"><td className="p-3 text-zinc-400">{formatDate(search.createdAt)}</td><td className="max-w-56 truncate">{search.query ?? "-"}{search.cacheHit ? " (caché)" : ""}</td><td>{search.status}</td><td>{search.resultCount}</td></tr>)}</tbody></table></div></div>
      </section>
      <section className="grid gap-6 xl:grid-cols-2">
