...
export function InterestPicker({ topics, initial }: { topics: InterestTopic[]; initial: number[] }) {
  ...
  return <div className="flex min-h-0 flex-col gap-7">
    <section className="rounded-2xl border border-white/10 bg-white/[.035] p-4 sm:p-5" aria-labelledby="selected-interests-title">
      ...
      {selectedCount > 0 ? <div className="mt-4 flex flex-wrap gap-2">{visibleTopics.filter((topic) => selected.has(topic.id)).map((topic) => <Button key={topic.id} variant="secondary" size="sm" className="max-w-full" onClick={() => toggle(topic.id)} aria-label={`Quitar ${topic.name}`}><span className="truncate">{topic.name}</span><X data-icon="inline-end" /></Button>)}{customInterests.map((interest) => <Button key={interest.key} variant="secondary" size="sm" className="max-w-full" onClick={() => setCustomInterests((current) => current.filter((item) => item.key !== interest.key))} aria-label={`Quitar ${interest.name}`}><span className="truncate">{interest.name}</span><X data-icon="inline-end" /></Button>)}</div> : <p className="mt-4 text-sm text-zinc-500">Parte por una categoría o agrega una señal propia.</p>}
    </section>
    <section aria-labelledby="categories-title"><div className="flex flex-col gap-1"><h3 id="categories-title" className="text-base font-semibold text-zinc-100">Explora por categoría</h3><p className="text-sm leading-6 text-zinc-400">Marca una categoría para priorizarla. Luego elige las señales concretas que más te interesan.</p></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">{categories.map((category) => { const Icon = categoryIcons[category.slug] ?? Aperture; const isSelected = selected.has(category.id); const isActive = activeCategory?.id === category.id; return <button key={category.id} type="button" onClick={() => { setActiveCategoryId(category.id); toggle(category.id); }} aria-pressed={isSelected} className="group flex min-h-24 flex-col items-start justify-between rounded-2xl border border-white/10 bg-white/[.025] p-3 text-left transition hover:border-white/30 hover:bg-white/[.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white aria-pressed:border-white/50 aria-pressed:bg-white/[.12]"><span className="flex w-full items-center justify-between"><Icon className="size-5 text-zinc-300 group-aria-pressed:text-white" /><span className={isActive ? "size-2 rounded-full bg-zinc-100" : "size-2 rounded-full bg-transparent"} aria-hidden="true" /></span><span className="mt-3 flex w-full items-end justify-between gap-2 text-sm font-medium leading-tight text-zinc-100"><span>{category.name}</span>{isSelected && <Check className="size-4 shrink-0 text-white" />}</span></button>; })}</div></section>
    <section className="border-t border-white/10 pt-6" aria-labelledby="signals-title"><div className="flex flex-wrap items-end justify-between gap-3"><div><h3 id="signals-title" className="text-base font-semibold text-zinc-100">{activeCategory ? `Afina ${activeCategory.name}` : "Afina tu radar"}</h3><p className="mt-1 text-sm text-zinc-400">Elige las actividades y estilos que quieres encontrar antes.</p></div>{activeCategory && <span className="text-xs font-medium text-zinc-500">{signals.length} señales</span>}</div>{signals.length ? <div className="mt-4 flex flex-wrap gap-2">{signals.map((signal) => { const isSelected = selected.has(signal.id); return <Button key={signal.id} onClick={() => toggle(signal.id)} aria-pressed={isSelected} variant={isSelected ? "default" : "outline"} size="sm" className="h-9 rounded-full">{isSelected && <Check data-icon="inline-start" />}{signal.name}</Button>; })}</div> : <p className="mt-4 text-sm text-zinc-500">No hay señales sugeridas para esta categoría todavía.</p>}</section>
    <section className="border-t border-white/10 pt-6" aria-labelledby="custom-interests-title"><div><h3 id="custom-interests-title" className="text-base font-semibold text-zinc-100">Agrega algo muy tuyo</h3><p className="mt-1 text-sm leading-6 text-zinc-400">Sigue artistas, bandas, juegos, sagas, fandoms o cualquier señal que no esté arriba.</p></div><div className="mt-4 grid gap-2 sm:grid-cols-[minmax(0,1fr)_12rem_auto]"><Input value={customName} onChange={(event) => setCustomName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addCustomInterest(); } }} maxLength={80} placeholder="Ej.: Mon Laferte, Dune, Fórmula 1" aria-label="Nueva señal personalizada" className="h-11 bg-white/[.03] text-zinc-100 placeholder:text-zinc-500" /><Select value={customKind} onValueChange={(value) => setCustomKind(value as CustomInterestKind)}><SelectTrigger aria-label="Tipo de señal personalizada" className="h-11 bg-white/[.03] text-zinc-200"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="artist">Artista o banda</SelectItem><SelectItem value="topic">Juego, saga u otro</SelectItem></SelectContent></Select><Button onClick={addCustomInterest} className="h-11"><Plus data-icon="inline-start" />Agregar</Button></div></section>
    <div className="sticky bottom-0 -mx-6 flex flex-col gap-3 border-t border-white/10 bg-popover/95 px-6 pb-1 pt-4 backdrop-blur sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-zinc-400" role="status" aria-live="polite">{message}</p><Button onClick={save} disabled={saving} className="shrink-0">{saving ? "Guardando…" : "Guardar cambios"}</Button></div>
  </div>;
}
